...
## Principles of ChatGPT {.incremental}
...
-   Input --\> Netzwerk mit Parametern --\> Output

```{mermaid}
flowchart LR
  X(X Input) --> N[Netzwerk]
  N --> Y(Output)
```
...
-   Wichtigkeit von bestimmten Eingangsdaten für ein Modelentscheidungen.[^3]
