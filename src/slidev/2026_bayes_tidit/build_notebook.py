"""Builds bayes_workflow.ipynb — the live part of the TIDIT Bayes talk.

Run:  ~/miniconda3/bin/python build_notebook.py
Then execute it (this is what checks that every cell runs, in order):
      ~/miniconda3/bin/jupyter nbconvert --to notebook --execute --inplace bayes_workflow.ipynb
"""
import os
...
nb.metadata["kernelspec"] = {"name": "miniconda", "display_name": "Python (miniconda)", "language": "python"}
md, code = nbf.v4.new_markdown_cell, nbf.v4.new_code_cell
C = []

C.append(md("""# Bayesian workflow: blood pressure and age
...
C.append(md("""## 1. Write down how the data could have come about

Four lines of numpy. Every number is a belief, written down *before* looking at the data."""))

C.append(code("""a     = np.random.normal(1, 5)          # SBP rises with age — how fast? not sure
...
C.append(md("""## 2. Prior predictive: run the story 200 times
...
C.append(md("""Negative blood pressure for a quarter of the patients. The priors *looked* harmless.

Tighter: `a ~ N(1, 0.4)`, `b ~ N(90, 15)` — go back up, change the numbers, re-run."""))

C.append(md("""## 3. Now look at the data"""))

C.append(code("""plt.scatter(age, sbp, color="k", s=20)
plt.xlabel("age"); plt.ylabel("systolic blood pressure [mmHg]");"""))

C.append(md("""## 4. Run the story backwards
...
C.append(code("""import numpyro, numpyro.distributions as dist
...
C.append(md("""**Forward** — no data passed, so `sbp=None`. Exactly the numpy loop:"""))

C.append(code("""prior = Predictive(model, num_samples=200)(random.PRNGKey(0), age)
...
C.append(md("""**Backward** — pass the data:"""))

C.append(code("""mcmc = MCMC(NUTS(model), num_warmup=1000, num_samples=2000, progress_bar=False)
...
C.append(md("""## 5. The same problem, the classical way"""))
...
nb.cells = C
out = os.path.join(HERE, "bayes_workflow.ipynb")
nbf.write(nb, out)
